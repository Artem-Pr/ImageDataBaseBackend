...
class FilePathsManager extends FilePathsBasic {
     ...
     */
    constructor(filedata, root, basePathWithoutRootDirectory) {
        ...
        this.saveFiledata(preparedFiledata)
    ...
    getPreviewPathWithoutRootDir(isFullSize) {
        const {isFullSizeJpgPath, targetName, thumbnailName} = this.filedata
        
        if (isFullSize && !isFullSizeJpgPath) return ''
    
        if (thumbnailName) return `/${this.getTargetPreviewBasePath(false)}${thumbnailName}`
        
        return `/${this.getTargetPreviewBasePath(isFullSize)}${this.getPreviewName(targetName, isFullSize)}`
    }
